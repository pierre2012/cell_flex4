...
def older(request, year=None):
	years = []
	if year:
		publications = Publication.objects.filter(year=2000, external=False)
	else:
		publications = Publication.objects.filter(year__lt=2000, external=False)
